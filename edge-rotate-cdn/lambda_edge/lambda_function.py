...
역할:
  ...
  4. cache_key = {hash}_{rotate} 로 구성 (회전별 캐시 분리)
  5. /image?hash=<cache_key>&image=<name>&rotate=<rotate> 로 리라이트
     → CloudFront가 hash 기준으로 캐싱
     → Viewer Response Lambda가 rotate 읽어서 응답 이미지 회전
"""
import json
import math
import struct
...
def lambda_handler(event, context):
    ...
    # hash_rotate 조합으로 캐시 키 구성 → 회전별 캐시 분리
